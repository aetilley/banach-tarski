...
lemma mod_lemma_Z (A : ZMAT) (v : Z3_raw) :
  mod7_Z (Matrix.mulVec A (mod7_Z v)) = mod7_Z (Matrix.mulVec A v) := by
...
(Matrix.diagonal (fun _ ↦ d))^k * M = M * (Matrix.diagonal (fun _ ↦ d))^k := by
  ...
  _ = M * (Matrix.diagonal (fun _ ↦ d))^k := by
    rw [←Matrix.diagonal_pow (fun _:(Fin 3) ↦ d) k]
...
lemma zip_lemma : ∀ N : ℕ, ∀ mats : List MAT, ∀ d : ℝ ,
    ...
    | succ N ih =>
      · intro mats d lhs
        ...
        have hprop : ∃H : MAT, ∃T : List MAT, mats = H::T := by
          exact List.exists_cons_of_length_eq_add_one lhs
        obtain ⟨H, T, pH⟩ := hprop
...
lemma sev_mat_Z_mod_is_zero_lemma (N : ℕ) : N ≠ 0 →
mod7_Z (Matrix.col (sev_mat_Z ^ N) 0) = 0 := by
  intro Nnz
  simp [sev_mat_Z]
...
lemma sevsevlem : (7:ℝ)⁻¹ • (7:ℝ)⁻¹  • (fun (_: Fin 3) ↦ (49:ℝ)) =
  fun (_:Fin 3) ↦ (1:ℝ) := by
  ext x
  simp
  norm_num
...
lemma isreduced_of_append_of_reduced_mismatching_boundary {α : Type u}
  {L M : List (α × Bool)} (p1 p2 : (α × Bool)) (pl : L.getLast? = some p1)
  (ph : M.head? = some p2) :
    (FreeGroup.IsReduced L) ∧ (FreeGroup.IsReduced M) ∧
    (p1.1 = p2.1 →  p1.2 = p2.2) → (FreeGroup.IsReduced (L ++ M)) := by
...
def leading_others_as_nat_f (w : List chartype) : ℕ :=
  Int.toNat (w.length - (n_trail_sinv_f w))
...
∃ wc : List chartype,
  ...
  --
    ...
    · exfalso
      rw [st] at pr
      rw [pr] at cset
      have bad: w = (List.take (leading_others_as_nat-1) w) ++ [(0, true)] ++
        List.replicate trailing_as_nat (0, false) := by
      ...
      have badbad: ¬ (FreeGroup.IsReduced w) := by
        ...
        let L2 := ((List.take (leading_others_as_nat - 1) w) ++
          (List.replicate trailing_as_nat.pred (0, false)))
  ...
  --
    · exfalso
      ...
      have wrong_bound: (n_trail_sinvs + 1) ≤ w.length ∧
        ...
        · have yyy:  ((0, false) :: List.drop (w.length - ↑max_fin) w)  =
            ([(0, false)] ++ List.drop (w.length - ↑max_fin) w):= rfl
          rw [yyy]
      ...
      have second_pred: List.drop (w.length - ↑(max_fin + 1)) w =
        List.replicate ↑(max_fin + 1) (0, false)  := by
...
lemma all_sinv_equiv (w : List chartype) : all_sinv_f w →
  w = List.replicate (w.length) (0, false) := by
  ...
  := by
    ...
    · simp [neq]
      ...
      · simp [flt]
        ...
        rw [←List.append_assoc]
        rw [←FreeGroup.reduce_append_reduce_reduce]
        have trt: FreeGroup.reduce (A ++ FreeGroup.invRev A) = [] := by
          exact reduce_invRev_right_cancel
        rw [trt]
        simp
...
∃ wc : List chartype,
  ...
  · calc w
      ...
      _ = FreeGroup.invRev (wc_inv ++ List.replicate (trailing_as_nat_f w_inv) (0, false)) := by
        nth_rewrite 1 [p_wc_inv.left]; rfl
      _ = FreeGroup.invRev (List.replicate (trailing_as_nat_f w_inv) (0, false)) ++
        FreeGroup.invRev (wc_inv) := FreeGroup.invRev_append
      _ = List.replicate (leading_s_as_nat_f w) (0, true) ++ FreeGroup.invRev wc_inv := by
        simp [FreeGroup.invRev]; exact leading_trailing
...
lemma wolog_zero {G : Type} [Group G] (h : FG2 →* G) (a : FG2) :
  (a ≠ 1) ∧ (h a = 1) → ∃a2 : FG2, (a2 ≠ 1) ∧ (h a2 = 1) ∧
  (FreeGroup.toWord a2).getLast? = some (0, true) := by
  ...
  let a2: FG2 :=  dite last_is_sigma (fun _ => a) (fun _ =>
    dite all_sinv (fun _ => a⁻¹) (fun _ =>
      σ ^ (-n_trail_sinvs - 1) * a * σ ^ (n_trail_sinvs + 1)))
