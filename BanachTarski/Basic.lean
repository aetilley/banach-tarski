...
theorem hausdorff_paradox: ∃ D : Set R3, (D ⊆ S2 ∧ Countable D ∧ Paradoxical SO3 (S2 \ D)) := by
  ...
  have each_fixed_countable : ∀g ∈ SATO_sub_1, Countable {x ∈ S2 | g • x = x} := by
    ...
    rcases em (Nat.card case = 0) with eqz | nez
    sorry
    --

    have is_finite: Finite case := by
      exact (Nat.card_ne_zero.mp (nez)).right
...
theorem banach_tarski_paradox_B3: Paradoxical G3 B3 := by
  ...
  have pso3b3m : Paradoxical SO3_in_G3 B3min := by
    have that :_ :=paradoxical_preserved_by_iso B3min SO3_embed_G3 (Equiv.refl R3) SO3_G3_action_equiv banach_tarski_paradox_B3_minus_origin
    simp at that
    exact that
