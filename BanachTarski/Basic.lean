...
lemma equidecomposibility_trans
    ...
    Equidecomposible G M N → Equidecomposible G L N := by
...
Paradoxical G E → Equidecomposible G E F → Paradoxical G F := by
  ...
  -- gC ⊆ F ∧ gD ⊆ F ∧ Disjoint gC gD ∧
  --   Equidecomposible G gC F ∧ Equidecomposible G gD F
  constructor
  -- gC ⊆ F
  · intro gc gc_in_gC
    rw [← coverBs, Set.mem_iUnion]
  ...
  · let ACs := fun i ↦ C ∩ (As i)
    ...
    have equiCgC: Equidecomposible G C gC :=
      ⟨n, ACs, BGCs, pdACs, pdBGCs, coverACs, coverBGCs, g, pmap_restrict_c⟩
  ...
  have equiDgD: Equidecomposible G D gD :=
    ⟨n, ADs, BGDs, pdADs, pdBGDs, coverADs, coverBGDs, g, pmap_restrict_d⟩
...
lemma equidecomposible_of_supergroup_of_equidecomposible
    {X : Type*} (G : Type*) [Group G] [MulAction G X] (E F : Set X) :
...
lemma paradoxical_of_supergroup_of_paradoxical
    {X : Type*} (G : Type*) [Group G] [MulAction G X] (H : Subgroup G) (E : Set X) :
Paradoxical H E → Paradoxical G E := by
  rintro ⟨neE, C, D, csube, dsube, disjcd, equiHCE, equiHDE⟩
  have equiGCE: Equidecomposible G C E :=
    equidecomposible_of_supergroup_of_equidecomposible G C E H equiHCE
  have equiGDE: Equidecomposible G D E :=
    equidecomposible_of_supergroup_of_equidecomposible G D E H equiHDE
  exact ⟨neE, C, D, csube, dsube, disjcd, equiGCE, equiGDE⟩


def SelfParadoxical (G : Type*) [Group G] : Prop := Paradoxical G (Set.univ : Set G)

theorem paradoxical_preserved_by_iso
    {X Y : Type*} {G H : Type*} [Group G] [Group H] [MulAction G X] [MulAction H Y]
    (E : Set X) (iso : G ≃* H) (f : X ≃ Y) :
    (∀ x : X, ∀ g : G, (iso g) • (f x) = f (g • x)) →
    Paradoxical G E → Paradoxical H (f '' E) := by
  ...
  have equiHimCimE: Equidecomposible H imC imE :=
    ⟨n, fCAs, fCBs, pdfCA, pdfCB, coverfCA, coverfCB, igC, ipmapfC⟩
  have equiHimDimE: Equidecomposible H imD imE :=
    ⟨m, fDAs, fDBs, pdfDA, pdfDB, coverfDA, coverfDB, igD, ipmapfD⟩
...
theorem self_paradoxical_preserved_by_iso {G H : Type*} [Group G] [Group H] :
    SelfParadoxical G → G ≃* H → SelfParadoxical H := by
  ...
  have cond1 : ∀ x : G, ∀ g : G, (iso g) • (iso x)  = iso (g • x) := by
    intro x g
    simp
  have this: Paradoxical H (iso '' g_univ) :=
    paradoxical_preserved_by_iso g_univ iso iso.toEquiv cond1 selfparaG
...
lemma free_group_of_rank_two_is_self_paradoxical : SelfParadoxical FG2 := by
  ...
  -- ∃ C D, C ⊆ Set.univ ∧ D ⊆ Set.univ ∧ Disjoint C D ∧
  --   Equidecomposible FG2 C Set.univ ∧ Equidecomposible FG2 D Set.univ
...
-- Equidecomposible FG2 C Dom
  ...
  have disσσ: σDom ∩ σσinvDom  = ∅ := by
    ...
    have bad :_ := calc FreeGroup.toWord x
      ...
      _ = FreeGroup.reduce (FreeGroup.toWord (σ) ++ FreeGroup.toWord (σ⁻¹ * x)) := by
        exact FreeGroup.toWord_mul σ (σ⁻¹ * x)
      _ = FreeGroup.reduce ([((σchar), true)] ++ FreeGroup.toWord (σ⁻¹ * x)) := by
        rw [show σ = FreeGroup.of σchar from rfl, FreeGroup.toWord_of σchar,
          show σchar = (0: Fin 2) from rfl]
  ...
  have disττ: τDom ∩ ττinvDom  = ∅ := by
    ...
    have bad :_ := calc FreeGroup.toWord x
      ...
      _ = FreeGroup.reduce (FreeGroup.toWord (τ) ++ FreeGroup.toWord (τ⁻¹ * x)) := by
        exact FreeGroup.toWord_mul τ (τ⁻¹ * x)
      _ = FreeGroup.reduce ([((τchar), true)] ++ FreeGroup.toWord (τ⁻¹ * x)) := by
        rw [show τ = FreeGroup.of τchar from rfl, FreeGroup.toWord_of τchar,
          show τchar = (1: Fin 2) from rfl]
...
theorem paradoxical_of_action_of_self_paradoxical
    ...
    (∀ g : G, (f g) '' Dom ⊆ Dom) →
    SelfParadoxical G →
    (¬∃ fp : X, fp ∈ FixedPoints G Dom) →
    Set.Nonempty Dom →
    Paradoxical G Dom := by
    intro eclosed ⟨nEmp, A, B, asube, bsube, disjab, equiADom, equiBDom⟩ nofixed nonemptyDom
    ...
    have choice_function_exists: ∃ f : orbits_type → X, ∀ (x : orbits_type), f x ∈ id x :=
      Classical.axiom_of_choice nonempty
    ...
    have pdinter: ∀ (g h : G), (g ≠ h) → Disjoint (f g '' choice_set) (f h '' choice_set) :=
      by
      intro g h gneh
      apply Set.disjoint_iff.mpr
      ...
      have res0: MulAction.orbit G yg = MulAction.orbit G yh := by
        simp [MulAction.orbit_eq_iff.mpr same]
      ...
      have eq: yg = yh := calc yg
        _ = choice (orb_yg_t) := by
          have know: ∃ y : orbits_type, choice y = yg := by
            simp [choice_set] at pyg
            exact pyg.left
          ...
          have so: y = orb_yg_t := by
            ...
            obtain ⟨w, pw⟩ := y_prop
            rw  [←pw.right] at so1
            have same_orbs : MulAction.orbit G yg = MulAction.orbit G w :=
              MulAction.orbit_eq_iff.mpr so1
        ...
        _ = yh := by
          have know: ∃ y : orbits_type, choice y = yh := by
            simp [choice_set] at pyh
            exact pyh.left
          obtain ⟨y, p_yp_is_choice_y⟩ := know
          have so0: (choice y) ∈ (y: Set X) := by apply (pchoice y)
          ...
          have so: y = orb_yh_t := by
            ...
            obtain ⟨w, pw⟩ := y_prop
            rw  [←pw.right] at so1
            have same_orbs : MulAction.orbit G yh = MulAction.orbit G w :=
              MulAction.orbit_eq_iff.mpr so1
...
lemma absorption_lemma_2 {X : Type*} (G : Type*) [Group G] [MulAction G X] (E : Set X) (S : Set X) :
    (∃ ρ : G, (∀ i : ℕ, (f ρ)^[i] '' S ⊆ E) ∧ (∀ i : ℕ, Disjoint S ((f ρ)^[i+1] '' S))) →
    Equidecomposible G E (E \ S) := by
    rintro ⟨ρ, sube, disj⟩
    let D := ⋃ k, (f ρ)^[k] '' S
...
∀ S : Set R3, (S ⊆ S2 ∧ Countable S → Equidecomposible SO3 (S2 \ S) S2) := by
  ...
  have countbad: Countable (Bad F S) :=
    countable_bad_rots S axis ⟨subset_of_s2, countable_S, polelem1, polelem2⟩
...
theorem banach_tarski_paradox_B3_minus_origin : Paradoxical SO3 B3min := by
  ...
  let AB'map : ∀ (i : Fin m), (fun (x : R3) ↦ (gAB i) • x) '' As' i = Bs' i :=
    map_lemma m gAB As_as_S2 Bs_as_S2 ABmap
  let CD'map : ∀ (i : Fin n), (fun (x : R3) ↦ (gCD i) • x) '' Cs' i = Ds' i :=
    map_lemma n gCD Cs_as_S2 Ds_as_S2 CDmap

  let equiE'S2': Equidecomposible SO3 E' B3min :=
    ⟨m, As', Bs', pdAs', pdBs', coverAs', coverBs', gAB, AB'map⟩
  let equiF'S2': Equidecomposible SO3 F' B3min :=
    ⟨n, Cs', Ds', pdCs', pdDs', coverCs', coverDs', gCD, CD'map⟩
...
theorem banach_tarski_paradox_B3 : Paradoxical G3 B3 := by
  ...
  have pso3b3m : Paradoxical SO3_in_G3 B3min := by
    have that : _ :=
      paradoxical_preserved_by_iso B3min SO3_embed_G3 (Equiv.refl R3) SO3_G3_action_equiv
  ...
  have equi : Equidecomposible G3 B3 B3min := by
    exact absorption_lemma_3 G3 B3 {origin} ⟨skew_rot, countbad, orbit_containment⟩

  exact paradoxical_of_equidecomposible_of_paradoxical G3 B3min B3 pg3b3m
    (equidecomposibility_symm equi)
